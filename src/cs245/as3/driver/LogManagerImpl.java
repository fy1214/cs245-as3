...
public class LogManagerImpl implements LogManager {
	...
	//检查日志方法，自写
	public void check() {
		int offset = getLogEndOffset();
		for (int i = 0; i < offset; i = i + Record.len) {
			byte[] bytes = readLogRecord(i, Record.len);
			Record record = Record.deserialize(bytes);
			System.out.println(record.TXid + "," + record.key +"," + record.trueValue().length);
		}
	}
